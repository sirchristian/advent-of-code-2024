...
for x in range(0, length):
    ...
    for level in levels[1:]:
        if not safe and dampener_triggered:
            break
        if not safe and not dampener_triggered:
            print("Triggered Dampener okay to continue")
            dampener_triggered = True
            safe = True
    ...
    num_safe_levels = num_safe_levels if not safe else num_safe_levels + 1
